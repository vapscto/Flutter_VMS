...
import 'package:m_skool_flutter/manager/feehead/api/fill_head_api.dart';
...
class FeeHeadWiseTab extends StatefulWidget {
  final LoginSuccessModel loginSuccessModel;
  final MskoolController controller;
  const FeeHeadWiseTab(
      {super.key, required this.loginSuccessModel, required this.controller});

  @override
  State<FeeHeadWiseTab> createState() => _FeeHeadWiseTabState();
}

class _FeeHeadWiseTabState extends State<FeeHeadWiseTab> {
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: DropdownButtonFormField<
                            ...
                            iconSize: 30,
                            items: List.generate(
                              handler.selectedYear.length,
                              (index) => DropdownMenuItem<
                                  FeeDetailsSelectedYearValues>(
                                value: handler.selectedYear.elementAt(index),
                                child: Padding(
                                  padding:
                                      const EdgeInsets.only(top: 13.0, left: 5),
                                  child: Text(
                            ...
                            onChanged: (s) {
                              ...
                              GetFillHeadDataApi.instance.getFeeHead(
                                  miId: widget.loginSuccessModel.mIID!,
                                  asmayId: s!.asmaYId!,
                        ...
                        Obx(() {
                          return handler
                                  .isErrorOccuredWhileLoadingFillHeadDetails
                                  .value
                              ? const Center(
                                  child: ErrWidget(err: {
                                    "errorTitle": "An Error Occurred",
                              ...
                              : handler.isloadingFillHeadDetails.value
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              DataTable(
                                                  ...
                                                  rows: List.generate(
                                                      handler.feeheaddetails
                                                          .length,
                                                      (index) =>
                                                          DataRow(cells: [
                                                            DataCell(Text(
                                                                "${handler.feeheaddetails.elementAt(index).headname}")),
                                                            DataCell(Text(
                                                                "${handler.feeheaddetails.elementAt(index).recived}")),
                                                            DataCell(Text(
                                                                "${handler.feeheaddetails.elementAt(index).paid}")),
                                                            DataCell(Text(
                                                                "${handler.feeheaddetails.elementAt(index).concession}")),
                                                            DataCell(Text(
                                                                "${handler.feeheaddetails.elementAt(index).ballance}")),
                                                          ]))),
                                            ],
                                          ),
                                        ),
