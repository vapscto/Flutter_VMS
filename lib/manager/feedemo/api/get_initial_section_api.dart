import 'package:dio/dio.dart';
import 'package:m_skool_flutter/constants/api_url_constants.dart';
import 'package:m_skool_flutter/controller/global_utilities.dart';
import 'package:m_skool_flutter/main.dart';
import 'package:m_skool_flutter/manager/feedemo/controller/fee_demo_controller.dart';
import 'package:m_skool_flutter/manager/feedemo/model/category_sec_model.dart';

class GetinitialSectionApi {
  GetinitialSectionApi.init();

  static final GetinitialSectionApi instance = GetinitialSectionApi.init();

  getSection(
      {required String base,
      required int miId,
      required int asmcLId,
      required FeeDemoController controller,
      required int asmayId}) async {
    final Dio ins = getGlobalDio();
    String api = base + URLS.getDemosectionData;
    logger.d(api);

    if (controller.isErrorOccuredWhileLoadingclass.value) {
      controller.updateIsErrorOcuuredWhileLoadingclass(false);
    }
    controller.updateIsLoadingclass(true);
    try {
      final Response response = await ins.post(api,
          options: Options(headers: getSession()),
          data: {"MI_Id": miId,
           "ASMAY_Id": asmayId,
            "asmcL_Id":controller.selectedClass!.asmcLId!});

      logger.d({"MI_Id": miId, "ASMAY_Id": asmayId, "asmcL_Id":controller.selectedClass!.asmcLId!});

      if (response.data['fillsection'] == null) {
        controller.updateIsErrorOcuuredWhileLoadingclass(true);
        controller.updateIsLoadingclass(false);
        return;
      }

      final ClasssectionListmodel academicYear =
          ClasssectionListmodel.fromJson(response.data['fillsection']);
      if (academicYear.values!.isEmpty) {
        controller.section.clear();
      } else {
        controller.updatesection(academicYear.values!);
        controller.selectedsection  = academicYear.values!.first;
      }

      controller.updateIsLoadingclass(false);
      controller.updateIsErrorOcuuredWhileLoadingclass(false);
    } on Exception catch (e) {
      logger.e(e.toString());
      controller.updateIsErrorOcuuredWhileLoadingclass(true);
      controller.updateIsLoadingclass(false);
    }
  }
}


