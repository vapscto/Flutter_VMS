...
Future<void> getFinalAdavncedViewDetails(
  ...
  try {
    ...
    FinalViewAdvancDetails finalViewAdvancDetail =
        FinalViewAdvancDetails.fromJson(response.data['veiwdetails']);
    controller.finalViewAdvance.addAll(finalViewAdvancDetail.values!);
    controller.etPaidAmount.value.text =
        finalViewAdvancDetail.values!.first.paidAmount.toString();
    // logger.w(response.data['timeArray']['\$values'][0]['vtadaA_TotalSactionedAmount']);
    double sncAmount = response.data['timeArray']['\$values'][0]
