...
class TaskCreationHomeNew extends StatefulWidget {
  final LoginSuccessModel loginSuccessModel;
  final MskoolController mskoolController;
  final String previousScreen;
  // final bool showAppBar;
  const TaskCreationHomeNew({
  ...
  @override
  State<TaskCreationHomeNew> createState() => _TaskCreationHomeNewState();
}

class _TaskCreationHomeNewState extends State<TaskCreationHomeNew> {
  ...
  Future<void> pickFile(int index) async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
        allowedExtensions: (index == 0)
            ? ['xls', 'xlsx']
            : ['jpg', 'jpeg', 'png', 'doc', 'docx', 'xls', 'xlsx', 'pdf']);
    if (result != null && result.files.isNotEmpty) {
      XFile xFile = XFile(result.files.single.path!);
      _taskDepartController.addListBrowser[index].file = xFile;
      _taskDepartController.addListBrowser[index].FileName = result.names.first;
      setState(() {});
    }
  }

  Future<void> pickFileOthers(int index) async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
        allowedExtensions: [
          'jpg',
          'jpeg',
