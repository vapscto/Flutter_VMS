...
Future<AssetsListModel?> assetsList({
  required String base,
  required int miId,
  required int hrmeId,
}) async {
  ...
  var request = http.Request('POST', Uri.parse("$base${URLS.assetsList}"));
  try {
    request.body =
        json.encode({"MI_Id": miId, "HRME_Id": hrmeId, "Flag": "Dashboard"});
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      String responseBody = await response.stream.bytesToString();
      ...
      if (jsonResponse != null) {
        AssetsListModel assetsListModel =
            AssetsListModel.fromJson(jsonResponse);
        totalAmount = 0.0;
        for (var i in assetsListModel.employeedetails!.values!) {
          totalAmount += i.iNVTGRNPurchaseRate!.toDouble();
        }
        return assetsListModel;
      }
    } else {
      logger.i(response.reasonPhrase);
    }
