...
import 'package:m_skool_flutter/checkbook_approval/controller/detailed_todo_controller.dart';
import 'package:m_skool_flutter/checkbook_approval/model/getTaDaModel.dart';
...
Future<int> updateCheque(
  {
   ...
   final String apiUrl = "https://vmsstaging.vapssmartecampus.com:40015/api/CheckBookEntryApprovalFacade/ApprovalData/";
  //  final String apiUrl = base + URLS.staffGalleryClassApi;
    ...
    try {
      ...
      if (response.data['getadvancetada'] == null) {
        ...
        GetTaDaModel loadTaDaResponse = GetTaDaModel.fromJson(response.data['getadvancetada']);
        ...
        for(int i= 0;i<loadTaDaResponse.values!.length;i++){
          controller.tEControllerListOfSncAmount.add(TextEditingController(
            text: controller.getTaDaModelList.elementAt(i).vPAYVOUAppliedAmount.toString()
          ) );
          controller.tEControllerListOfNarration.add(TextEditingController(
            text: controller.getTaDaModelList.elementAt(i).vPAYVOURemarks.toString()
          ));
          controller.tEControllerListOfApprovalRemark.add(TextEditingController(
            text:  controller.getTaDaModelList.elementAt(i).vPAYVOUToSupplierFlg != null ? "" :"",
          ));
          controller.checkList.add(controller.getTaDaModelList.elementAt(i).hRMBDInterbankTransFlg!);
          controller.radioSelect.add(controller.getTaDaModelList.elementAt(i).vPAYVOUStatusFlg.toString() == "Rejected" ? 0 :1);
        }
